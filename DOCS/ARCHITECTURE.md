...
- cross-db FK 대신 애플리케이션 레벨 참조를 사용한다.
...
- 운영에서는 split DB URL 누락 시 fail-fast 한다.
...
## 11. `timetable_db`에서 빠져야 하는 것
- profile-like 필드
- legacy `CalendarEvent`
- legacy `TaskItem`
- 장기적으로는 local `User` 자체

## 12. 남은 단계
1. `projects_db`와 프로젝트 모듈 구현
2. timetable ownership을 `authUserId` 중심으로 더 단순화
3. timetable legacy `User.password` 제거 수순 정리
4. `timetable_db`에 남아 있는 legacy 테이블 정리
5. split DB 검증 자동화 보강

## 13. 완료 기준
- `workspace_auth`는 인증 데이터만 가진다.
- `workspace_core`는 공통 프로필/모듈 상태만 가진다.
- `timetable_db`는 시간표 데이터만 가진다.
- `calendar_db`는 캘린더 데이터만 가진다.
- `tasks_db`는 할 일 데이터만 가진다.
- 새 모듈은 기본적으로 별도 DB 경계에서 시작한다.
